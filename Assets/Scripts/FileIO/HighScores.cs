...
public class HighScores : MonoBehaviour
{
    ...
    public void LoadScoresFromFile()
    {
        ...
        while (fileReader.Peek() != 0 && scoreCount < scores.Length)
        {
            ...
            if (didParse)
            {
                //If we succesfully read a number, put it in the array.
    ...
    {
        //First up we find out what index it belongs at. This will be the first index with a score lower then the new score.
        int desiredIndex = -1;
        for (int i = 0; i < scores.Length; i++)
        {
